...
double CalcMatMulTime_ijk(double* A, double* B, double* C, size_t N) {
    ...
    for (i = 0; i < N; i++)
        for (j = 0; j < N; j++) {
            for (k = 0; k < N; k++) C[i * N + j] = C[i * N + j] + A[i * N + k] * B[k * N + j];
        }
...
double CalcMatMulTime_kij(double* A, double* B, double* C, size_t N) {
    ...
    for (k = 0; k < N; k++)
        for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) C[i * N + j] = C[i * N + j] + A[i * N + k] * B[k * N + j];
        }
    gettimeofday(&end, NULL);
...
double CalcMatMulTime_kij_opt(double* A, double* B, double* C, size_t N) {
    ...
    for (k = 0; k < N; k++)
        for (i = 0; i < N; i++) {
            dummy = i * N;
            for (j = 0; j < N; j++) C[dummy + j] = C[dummy + j] + A[dummy + k] * B[k * N + j];
        }
    gettimeofday(&end, NULL);
...
double CalcMatMulTime_kij_openmp(double* A, double* B, double* C, size_t N) {
    struct timeval start, end;
    double r_time = 0.0;
...
int main() {
    ...
    average_runtime = 0.0;
    for (int n = 0; n < NRuns; n++) {
        runtimes[n] = CalcMatMulTime_kij_openmp(&A[0], &B[0], &C[0], N);
        printf("runtime %lf seconds\n", runtimes[n]);
        average_runtime += runtimes[n] / NRuns;
    }

    printf("average runtime kij with OpenMP %lf seconds\n", average_runtime);
    printf("---------------------------------\n");
